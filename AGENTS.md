...
## Current phase
The current phase is the **price comparator**: direct Steam price + ITAD official/authorized offers + gg.deals retail/keyshops aggregate + FX conversion to MXN (`docs/PLAN_BASE_MVP.md`, `docs/PLAN_ITAD.md`, `docs/PLAN_GGDEALS.md`). Multi-store direct pricing (Epic and Microsoft/Xbox; no IGDB, ITAD demoted to identity resolver + fallback and always `pricing_type='fx_estimate'`) lives in `docs/PLAN_MULTISTORE.md`: Fases 0, 1 and 2 are implemented, including `IStorePriceProvider`/`EpicStoreClient`, `MicrosoftStoreClient`, the `itad.link` redirect resolver, `steam_games.epic_refreshed_at` and `POST /api/library/prices/sync` (the bounded pass that prices the Xbox library rows Steam cannot price, one provider call per game). Fase 1's two migrations (`2026-10-02_game_offers_canonical_anchor.sql`, `2026-10-03_epic_store_offers.sql`) **are applied on the server** and `SQL/checks/game_offers_anchor.sql` returns 0 rows; Fase 1 is verified in production (the game detail shows the Epic row at native MXN 161.99, not an FX estimate). Committed in `865ec95`. Fase 2's migration (`2026-10-04_game_offers_steam_less.sql`, plus `2026-10-05_steam_games_microsoft_refreshed_at.sql`) is **not** applied yet: the first makes `game_offers.steam_game_id` nullable so an offer can hang off `game_id` alone, the second adds the Microsoft refresh gate. Microsoft has **three identity paths** (`docs/PLAN_MULTISTORE.md` §4.2): the PackageFamilyName of an Xbox library row (`products/lookup`), a StoreId taken from the Microsoft deal link ITAD returns (`products/{StoreId}`) — the second is what prices a game the user does not own on Xbox, and it is written during the game detail refresh exactly like Epic's — and, since ITAD lists no Microsoft shop for every game it knows, the store's own search as a last resort (`FindOfferByTitleAsync`, allowed only when the ITAD phase answered and is not degraded). Epic has the same three: a stored slug, the slug read out of ITAD's Epic link, and the same title search. Two measured traps live in that title path: `steam_games.name` is stored **localized** (`cc=mx&l=spanish`), so Epic's search does not know `Caballeros Floppy` while Microsoft's catalog does; and Epic does not put the readable slug in `urlSlug` for every game (`Floppy Knights` returns a hash there and `floppy-knights-6a735a` only in `offerMappings[].pageSlug`), so a slug match has to check all four slug-bearing fields. Both are `StoreTitleMatcher`/`EpicStoreClient` doc comments with the measured case attached. `IStorePriceProvider` stays mapped to Epic only — a second unkeyed registration would make the last one win for every consumer, so both the Xbox library pass and `SteamGameService`'s Microsoft phase take `MicrosoftStoreClient` by type. Epic's GraphQL endpoint answers **403 without a User-Agent** (`Epic:UserAgent`) — necessary but not sufficient: that endpoint's Cloudflare WAF also classifies clients by TLS fingerprint, and OpenSSL 3.0.x gets a challenge while ≥3.3 passes. That is why the runtime stage of `Dockerfile.api` is `aspnet:9.0-azurelinux3.0` and not `aspnet:9.0`. Do **not** measure this from the host with `curl` (`docs/PLAN_MULTISTORE.md` §2.1); measure inside the runtime image. Microsoft needs no such workaround: its catalog answers the server, the container and any client. External bundles have their own V1 **display-only** implementation on the same game detail (`docs/PLAN_BUNDLES.md`): ITAD `games/overview/v2` discovery + `external_bundles`/`external_bundle_games`. Bundles never enter `game_offers` or any savings math, and the summary's bundle card (`pickPricedBundleTier`) only reports the bundle's own price. The game detail's "best comparable price" section paints **one card per provider that has comparable offers**, showing **that provider's** cheapest price — never Steam's — with the **lowest price of all cards in green** (an `fx_estimate` competes for it), labelled `Más barato que Steam` when it beats Steam's direct price and `El más barato de las tiendas` when it does not; the `≈` on the price and the row's classification badge already say where the figure comes from, and Steam's own price is the page headline and the referee, not a card. That rule exists because the same mislabelled card happened twice by two different doors: first a group with **no** offers inheriting Steam's price, then a group where **Steam won the comparison**, both painting a store's name over Steam's price and link. Price and its label must always come from the same source. A third, milder version: while green meant "beats Steam", a page where Steam was cheaper (the common case — ITAD quotes USD and Steam's Mexican regional price is usually lower) showed no green card at all and read as broken. A fourth: restricting green to **regional** prices so a conversion would not outrank a store price put the green on Epic at 71.99 MXN while gg.deals showed 16.56 and ITAD 20.69 for Floppy Knights (1057800) — both estimates, both lower. Green over a card that is more expensive than another on the same screen reads as broken arithmetic. Savings calculation is still not implemented, and no live MX fixture has certified the parser yet — treat the ITAD bundle contract as pending runtime validation. The Playnite library is implemented with per-game favorites (`user_game_favorites`), Steam cover art filled into `games.image_url` (`POST /api/library/covers/sync`, bounded pass; `PUT /api/games/{gameId}/cover` for a manual pick — URL only, never identity or prices) and multiple reviews per `(game, platform)` carrying a mandatory `status` (`finished | completed | dropped`; "por jugar" is the absence of a review). Future plans live in `docs/`: canonical games (`PLAN_CATALOG.md`), Playnite library/reviews (`PLAN_LIBRARY.md`), wishlist (`PLAN_WISHLIST.md`), and the verified export procedure (`PLAYNITE_EXPORT.md`). `docs/dealext-library.json` is a personal import fixture and must remain ignored.
...
## Diagnóstico de un proveedor externo

Cuatro reglas, cada una pagada con una ronda de depuración en producción (`docs/PLAN_MULTISTORE.md` §8).

1. **Antes de proponer un rebuild, agota lo que la BD puede probar.** Cada fase directa sella su propio
   timestamp (`epic_refreshed_at`, `microsoft_refreshed_at`) y el pase de biblioteca usa
   `game_offers.observed_at`. Un timestamp **nuevo** con **0 filas** de esa tienda no significa "el binario
   es viejo": significa "la fase corrió y no encontró identidad". Son dos causas distintas con el mismo
   síntoma, y una consulta las separa. Pedir esa consulta es el primer paso, no el último.
2. **Verifica toda constante contra su fuente antes de escribirla, y deja el comando al lado.** Los ids de
   proveedor se copian de un sitio donde están en fila, y un id equivocado **no falla**: el proveedor
   simplemente no aparece en el payload, el enlace nunca se lee, y la fase reporta "no lo vende" con el
   mismo aspecto que un juego que de verdad no está. Si el valor se puede comprobar con una llamada de
   lectura, se comprueba (p. ej. los shop ids de ITAD: `service/shops/v1?country=MX`).
3. **Cuando la causa sea un dato equivocado tuyo, escribe la corrección donde el siguiente la lea.** En el
   comentario de la constante o en la tabla de riesgos del plan, con el síntoma exacto y el comando que lo
   comprueba. Un "ya quedó" no evita la cuarta ronda.
4. **Toda sonda necesita un control positivo: el mismo comando sobre un caso que se sabe que funciona, y
   el resultado esperado escrito al lado.** Una sonda mal escrita no falla: devuelve el vacío con el que se
   confunde un "no existe". Tres formas de escribirla mal se cobraron una ronda cada una, y las tres
   devolvían un resultado plausible: el **título localizado** (`steam_games.name` está en español, así que
   buscar en la tienda con él no encuentra nada), la **clave del proveedor** sin su forma correcta (ITAD
   quiere `app/<appid>` para Steam, no el id pelado: contesta `null` sobre un juego que sí tiene) y el
   **identificador equivocado** (`StorePageMapping.productId` no es el offer id de `catalogOffer`: 404 sobre
   un juego existente). Con el control, las tres se leen como "la sonda está mal" en el primer intento, no
   en el tercero. Ejemplos con su salida esperada en §10 del plan.

Corolario para lo medible: una verificación que corre en el cliente equivocado no es una verificación
(§2.1 del plan: `curl` desde el host no dice lo que hará el contenedor).
